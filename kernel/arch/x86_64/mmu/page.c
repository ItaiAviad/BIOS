...
void map_page(uint64_t* pml4, PageFrameAllocator* allocator, uint64_t virtual_address, uint64_t physical_address, uint64_t flags) {
    ...
    allocator->bitmap[(uint64_t)pml4/PAGE_SIZE] = 1; // Map page managment structure as in use
    ...
    if (!(pml4[pml4_index] & PAGE_PRESENT)) {
        ...
        allocator->bitmap[(uint64_t)pdpt/PAGE_SIZE] = 1;
    }
    if (!(pdpt[pdpt_index] & PAGE_PRESENT)) {
        ...
        allocator->bitmap[(uint64_t)pd/PAGE_SIZE] = 1;
    }
    if (!(pd[pd_index] & PAGE_PRESENT)) {
        ...
        allocator->bitmap[(uint64_t)pt/PAGE_SIZE] = 1;
    }
