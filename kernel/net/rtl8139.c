...
void rtl8139_init(void) {
    ...
    nic.rx_buf = (uint32_t *)malloc(RX_BUFFER_SIZE);
    ...
    outl(ioaddr + RTL8139_OFFSET_RCR, RTL8139_RCR_ACCEPT_ALL | (1 << 7)); // (1 << 7) is the WRAP bit, 0xf is AB+AM+APM+AAP
...
void rtl8139_read_self_mac(void) {
    for (uint16_t i = 0; i < sizeof(nic.mac); i++)
        nic.mac[i] = inb(nic.ioaddr + RTL8139_OFFSET_MAC0 + i);
}
...
void receive_packet() {
    ...
    struct eth_packet pkt = {
        ...
        .len = *(pkt.header + 2) - RTL8139_RX_CRC_LEN, // CRC is included in len, subtract it
    ...
    rx_offset = (rx_offset + pkt.len + 0xb) & ~3;                 // Align to 4-byte boundary
    ...
    outw(nic.ioaddr + RTL8139_OFFSET_CAPR, rx_offset - 16); // RTL8139 expects offset - 16
}
